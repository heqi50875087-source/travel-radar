...
(function (TR) {
  ...
  function renderProfile(root, c) {
    ...
    const amap = (name) => E().amapLink(name, c.id);
    const linkRows = (arr, nav) => `<div class="list-rows">${arr.map((x) => `<div class="row"><span>${esc(x)}${nav ? ` <a class="nav-a" target="_blank" rel="noopener" href="${amap(x)}">导航</a>` : ""}</span></div>`).join("")}</div>`;
    const chipFlow = (arr, nav) => `<div class="chips-flow">${arr.map((x) => nav
      ? `<a class="item link" target="_blank" rel="noopener" href="${amap(x)}" title="在高德中查看">${esc(x)}</a>`
      : `<span class="item">${esc(x)}</span>`).join("")}</div>`;
    ...
    const blocks = [];
    const B = (cls, ico, title, count, inner) => blocks.push(`<section class="card block ${cls} rv"><h3><span class="b-ico">${ico}</span>${title}${count ? ` <span class="b-count">×${count}</span>` : ""}</h3>${inner}</section>`);

    if (d && d.radar && d.radar.quickGlance) {
      const q = d.radar.quickGlance;
      B("wide", "🧭", "速览", "", `<div class="list-rows">${q.bullets.map((b) => `<div class="row"><span>${esc(b)}</span></div>`).join("")}</div>
        <div class="chips-flow" style="margin-top:12px">${(q.tags || []).map((t) => `<span class="tag brass">${esc(t)}</span>`).join(" ")}</div>`);
    }
    if (monthsHtml) B("full", "📅", "十二月历", "", `<div class="months-strip">${monthsHtml}</div><p class="desc" style="margin-top:4px">红框=当前月 · 金边=最佳季（${esc((c.seasons || []).join("、"))}）</p>`);
    if (d && d.mustEat2 && d.mustEat2.length) B("", "🍜", "必吃", d.mustEat2.length, chipFlow(d.mustEat2, true));
    if (solo.easy.length || solo.hard.length) B("", "🥢", "一人食指北", "", `<div class="solo-cols">
      ${solo.easy.length ? `<div><h4>一个人吃刚刚好</h4>${chipFlow(solo.easy)}</div>` : ""}
      ${solo.hard.length ? `<div class="hardline"><h4>大菜 · 拼午市或例份</h4>${chipFlow(solo.hard)}</div>` : ""}
      </div>${solo.tip ? `<p class="solo-tip">💡 ${esc(solo.tip)}</p>` : ""}`);
    if (d && d.oldShops && d.oldShops.length) B("", "🏮", "老字号", d.oldShops.length, linkRows(d.oldShops, true));
    if (d && d.localFav && d.localFav.length) B("", "❤️", "本地人心水", d.localFav.length, linkRows(d.localFav));
    if (d && d.highlights2 && d.highlights2.length) B("wide", "🗺", "看点", d.highlights2.length, chipFlow(d.highlights2, true));
    if (d && d.smallStreets && d.smallStreets.length) B("", "🏘", "老街小巷", d.smallStreets.length, linkRows(d.smallStreets, true));
    if (d && d.phenology && d.phenology.length) B("wide", "🌸", "物候历", d.phenology.length, `<div class="pheno-grid">${d.phenology.map((p) => `
      <div class="pheno"><div class="p-name">${esc(p.icon || "🌿")} ${esc(p.name)}</div><div class="p-when">${esc(p.when || "")}</div><div class="p-where">${esc(p.where || "")}</div></div>`).join("")}</div>`);
    if (d && d.wildlife && d.wildlife.length) B("", "🐦", "野生小友", d.wildlife.length, `<div class="pheno-grid">${d.wildlife.map((w) => `
      <div class="pheno"><div class="p-name">${esc(w.icon || "🐾")} ${esc(w.name)}</div><div class="p-where">${esc(w.where || "")}</div>${w.tip ? `<div class="p-when">${esc(w.tip)}</div>` : ""}</div>`).join("")}</div>`);
    if (d && d.radar && d.radar.travel) {
      const t = d.radar.travel;
      B("wide", "🚄", "抵达与内部交通", "", `<div class="list-rows">
        ...
        ${t.tips && t.tips.length ? `<h4 style="margin:14px 0 9px;font-size:13.5px">📌 交通与实操提示</h4><div class="list-rows">${t.tips.map((x) => `<div class="row"><span>${esc(x)}</span></div>`).join("")}</div>` : ""}`);
    }
    if (d && d.radar && d.radar.tips) {
      const tp = d.radar.tips;
      if (tp.avoid && tp.avoid.length) B("wide avoid", "🛡", "防坑清单", tp.avoid.length, linkRows(tp.avoid));
      if (tp.etiquette && tp.etiquette.length) B("etiq", "🤝", "入乡随俗", "", linkRows(tp.etiquette));
    }
    if (d && d.inspiration) {
      const ins = d.inspiration;
      const seg = [];
      if (ins.locals && ins.locals.length) seg.push(`<h4 style="margin:0 0 9px;font-size:13.5px;color:var(--pine)">本地人才知道</h4>${linkRows(ins.locals)}`);
      if (ins.hidden && ins.hidden.length) seg.push(`<h4 style="margin:14px 0 9px;font-size:13.5px;color:var(--brass)">游客罕至</h4>${linkRows(ins.hidden)}`);
      if (ins.routes && ins.routes.length) seg.push(`<h4 style="margin:14px 0 9px;font-size:13.5px;color:var(--terra)">串线参考</h4>${linkRows(ins.routes)}`);
      if (seg.length) B("full", "✨", "灵感 · 换个玩法", "", seg.join(""));
    }
    if (!d) {
      B("wide", "🗂", "基础档案", "", `<div class="list-rows">
    ...
    // 笔记
    B("full", "📝", "我的笔记", "", `<textarea class="note-area" id="cityNote" placeholder="写点什么：想吃的店、朋友的建议、踩过的坑……只保存在你自己的设备里">${esc(S.notes[c.id] || "")}</textarea>`);

    body.innerHTML = `
      <section class="profile-hero contour">
        <h1 class="cityname">${esc(c.id)}<small>${esc(c.region)} · ${esc(c.province)}${c.country && c.country !== "中国" ? " · " + esc(c.province) : ""}</small></h1>
        ...
        <div class="profile-meta">
          ...
          ${d ? `<span class="tag terra">深度档案 ●</span>` : ""}
        </div>
